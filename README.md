...
> [!NOTE]\
> 如果需要对本地视频源进行推流，可在`config`目录下新建`live`或`hls`（推荐）文件夹\
> live文件夹用于推流live接口，hls文件夹用于推流hls接口\
> 将以`频道名称命名`的视频文件放入其中，程序会自动推流到对应的频道中\
> 可访问 http://localhost:8080/stat 查看实时推流状态统计数据
